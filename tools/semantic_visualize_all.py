...
if __name__ == "__main__":
    ...
    parser.add_argument('--black_bg', '-bl', type=int, choices=[0, 1],
                        default=0, help="use black or white background for visualization")
...
#         raise Exception("An image should be chosen.")
    ...
    ## load gt_label
    for image_id in range(1, 50+1):
        label_name = 'sample_' + str(image_id) + '_class.mat'  # e.g. sample_1_class.mat
        ...
        visualize_semantic_segmentation(label, colorMap, black_bg=black_bg,
                                    save_path=os.path.join(visualize_save_base_dir, str(image_id) + '.png'))
