...
class DeepLab(object):
    ...
    def __init__(self, batch_size=1,
                 ...
                 images=None, #tf.placeholder(tf.float32, shape=(1,750,750,3)),
                 labels=None, #tf.placeholder(tf.int32),
    ...
    """
        ...
        self.labels = labels
        if self.images == None:
            self.images = tf.placeholder(tf.float32, shape=(1,None,None,3))
        if self.labels == None:
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('fc_final_sketch46'):
            ...
            x_group1 = self._conv('shortcut1', x_group1, 2, 64, self.num_classes, self._stride_arr(2), bias=True) 
            x = tf.add(x, x_group1)
    ...
    def _relu(self, x, leakiness=0.0):
        ...
        return tf.nn.relu(x, name='leaky_relu')
