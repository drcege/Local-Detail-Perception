...
def generate_edgelist(image_id, data_type, sketchyscene_base_dir):
    assert data_type in ['test', 'val']
    drawing_base_path = os.path.join(sketchyscene_base_dir, data_type, 'DRAWING_GT')
    edgelist_mat_path = os.path.join(sketchyscene_base_dir, data_type, 'edgelist','edgelist_' + str(image_id) + '.mat')
...
def refine_label_with_edgelist(image_id, dataset_type, data_base_dir,
                              origin_label, pixels_percent_threshold=0.5):
    """
    very slow!!
    ...
    """
    ## first generate edgelist
    edgelist_label = generate_edgelist(image_id, dataset_type, data_base_dir)  # (750, 750)
#     edgelist_label = label2d_array_nn_scaling(edgelist_label,
#                                               origin_label.shape[0], origin_label.shape[1])  # (768, 768)
    assert edgelist_label.shape == origin_label.shape

    nEdgeList = int(np.max(edgelist_label))
    print('nEdgeList', nEdgeList)

    refined_label = origin_label.copy()

    for j in np.unique(edgelist_label)[1:]:  #ignore background segment
        label_t = refined_label.copy()
        label_t[edgelist_label != j] = 0
        label_count = np.bincount(label_t.flatten())
        if len(label_count) > 1:
            label_count[0] = -1  # explicitly avoid chosen
            cls = np.argmax(label_count)
            edge_pixels_count = np.sum(edgelist_label == j)
            if label_count[cls] / edge_pixels_count >= pixels_percent_threshold:
                # print('refined here!')
                refined_label[edgelist_label == j] = cls
